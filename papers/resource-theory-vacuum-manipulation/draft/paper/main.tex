...
%     (PRA 109, 062208) in full. They are journal-only; the Related-work paragraph
%     below describes them from their abstracts and from how Refs. [Xie2025, Yan2026]
%     cite them. Check in particular whether their "QET passive" states coincide with
...
%  3. Confirm or edit the AI-assistance disclosure in the acknowledgments.
...
$\WCK$, has a closed form (Proposition~\ref{prop:closed}). It is built on the conditional
Hamiltonian that Xie, Sajjan, and Kais call Bob's local effective Hamiltonian~\cite{Xie2025}.
For the minimal model it gives Hotta's optimum in the compact form
$E_B^{\max}=\sqrt{h^2+4k^2}-(h^2+2k^2)/\sqrt{h^2+k^2}$.
...
\emph{et al.}~\cite{Fan2024steering} analyse QET with the correlation matrix, relate it to
quantum steering, and define ``QET passive'' states. Those states are the analogue of our free
states; their class of allowed operations may differ. Fan \emph{et al.}~\cite{Fan2024resources}
study how quantum resources affect the efficiency of QET and give necessary and sufficient
conditions for the minimal model. As far as we know, none of these works formulates a class
of free operations with a ledger monotone, characterizes the free class, or addresses two-way
communication.
...
[Acknowledgments to be supplied.] [Draft disclosure, to be confirmed or edited by the author:
the analysis code, the numerical checks, and a first draft of this manuscript were produced
with the assistance of an AI system (Claude, Anthropic); the author is responsible for the
content.]
\end{acknowledgments}
